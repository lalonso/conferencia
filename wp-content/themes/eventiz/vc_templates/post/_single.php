...
<?php
  ...
  if($categories){
    foreach($categories as $category) {
      $output .= '<a href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( esc_html__( "View all posts in %s", 'eventiz' ), $category->name ) ) . '">'.$category->cat_name.'</a>'.$separator;
    }
    ...
    <div class="entry-content">
        ...
        <?php
            if (! has_excerpt()) {
                ...
                ?>
                    <p class="entry-description"><?php echo eventiz_fnc_excerpt(135,'...'); ?></p>
                <?php
            }
        ?>
