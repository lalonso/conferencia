...
if( !class_exists("Eventiz_PBR_bootstrap_navwalker") ){
class Eventiz_PBR_bootstrap_navwalker extends Walker_Nav_Menu {
   ...
   /**
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if( isset($item->text_customize) && !empty($item->text_customize) ){
            switch ( $item->text_customize ) {
                case 'text_new':
                    $text_customize  = esc_html__('New', 'eventiz');
                    break;

                case 'text_hot':
                    $text_customize  = esc_html__('Hot', 'eventiz');
                    break;

                case 'text_featured':
                    $text_customize  = esc_html__('Featured', 'eventiz');
                    break;
    ...
    public function genMegaMenuByConfig( $item, $depth ){
        ...
        $post = get_post( $item->megamenu_profile ); $pbrmegamenu = 'true';
