...
if( !function_exists("eventiz_fnc_wpml_language_buttons") ){
   function eventiz_fnc_wpml_language_buttons(){
  ...
  */
function eventiz_fnc_get_header_layouts(){
    $path = get_template_directory().'/header-*.php';
    $files = glob( $path  );
    $headers = array( '' => esc_html__('Default', 'eventiz') );
    if( $files && is_array( $files) ){
        foreach ($files as $key => $file) {
            $header = str_replace( "header-", '', str_replace( '.php', '', basename($file) ) );
            $headers[$header] = esc_html__( 'Header', 'eventiz' ) . ' ' .str_replace( '-',' ', ucfirst( $header ) );
        }
    }
  ...
  */
function eventiz_fnc_get_footer_profiles(){
    
    $footers_type = get_posts( array('posts_per_page' => -1, 'post_type' => 'footer') );
    $footers = array(  '' => esc_html__('None', 'eventiz') );
 ...
 */
function eventiz_fnc_get_menugroups(){
 ...
 *
 */
function eventiz_fnc_cst_skins(){
 ...
 * 
 */
function eventiz_fnc_render_post_content( $footer ){

  global $eventiz_wpopconfig; 

	$post = get_post( $footer );
  $eventiz_wpopconfig['type'] = 'footer';
	if($post){
    	echo do_shortcode( $post->post_content ); 
  }
  
  $eventiz_wpopconfig['type'] = '';
 ...
 */
if(!function_exists('eventiz_fnc_makeid')){
    function eventiz_fnc_makeid($length = 5){
...
if(!function_exists('eventiz_fnc_excerpt')){
    //Custom Excerpt Function
    function eventiz_fnc_excerpt($limit,$afterlimit='[...]') {
...
function eventiz_fnc_get_widget_block_styles(){
