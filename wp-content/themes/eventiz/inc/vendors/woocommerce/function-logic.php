 ...
 * --------------------------------------------------------------------------- */
function eventiz_fnc_woocommerce_query($type,$post_per_page=-1,$cat='', $sort = 'desc'){
...
function eventiz_fnc_get_review_counting(){
...
function eventiz_fnc_price( $price ){
    return $price;
}


function eventiz_fnc_woocommerce_getcategorychilds( $parent_id, $pos, $array, $level, &$dropdown ) {
    ...
    for ( $i = $pos; $i < count( $array ); $i ++ ) {
        if ( isset($array[ $i ]) && $array[ $i ]->category_parent == $parent_id ) {
            ...
            eventiz_fnc_woocommerce_getcategorychilds( $array[ $i ]->term_id, $i, $array, $level + 1, $dropdown );
        }
    }
}
